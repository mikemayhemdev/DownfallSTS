...
public class GremlinSack extends CustomRelic {
    ...
    public String getUpdatedDescription() {
        if (AbstractDungeon.player != null) {
            // //downfallMod.logger.info("passed player check");
            if (sackCards != null) {
                //  //downfallMod.logger.info("passed null check - " + sackCards.size());
                if (sackCards.size() >= 5) {
                    //   //downfallMod.logger.info("passed count check - " + sackCards.size());
