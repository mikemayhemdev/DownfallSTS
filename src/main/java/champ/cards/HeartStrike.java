...
public class HeartStrike extends AbstractChampCard {
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        if (p.hasPower(VigorPower.POWER_ID)) {
            this.baseDamage = p.getPower(VigorPower.POWER_ID).amount;
    ...
    public void applyPowers() {
        if (AbstractDungeon.player.hasPower(VigorPower.POWER_ID)) {
            baseDamage = Math.min(AbstractDungeon.player.getPower(VigorPower.POWER_ID).amount, AbstractDungeon.player.currentHealth - 1);
        }
        super.applyPowers();
        if (AbstractDungeon.player.hasPower(VigorPower.POWER_ID)) {
